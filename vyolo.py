...
# 修改输入图片文件夹
img_folder = "D:/H/THU/paper/data/Data-Generation/output/leaftissue/"
img_list = os.listdir(img_folder)
img_list.sort()
# 修改输入标签文件夹
label_folder = "D:/H/THU/paper/data/Data-Generation/output/leaftissuelabels/"
label_list = os.listdir(label_folder)
label_list.sort()
# 输出图片文件夹位置
path = os.getcwd()
# output_folder = path + '/' + str("output")
output_folder = "D:/H/THU/paper/data/Data-Generation/output/vleaftissueoutput/"
...
if __name__ == '__main__':
    for i in range(len(label_list)):
        ...
        image_path = img_folder + "/" + label_list[i].split('.')[0] + ".jpg"
        #如果不存在匹配标签文件则跳过
        # if img_list[i].split('.')[0] != label_list[i].split('.')[0]:
        #     continue
        ...
        cv2.waitKey(0)
        """
        cv2.imwrite(output_folder + '/' + '{}.jpg'.format(image_path.split('/')[-1][:-4]), img)
