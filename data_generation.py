...
class SyntheticImageGenerator:
    ...
    def __init__(self, input_dir: str, output_dir: str, image_number: int, max_objects_per_image: int, image_width: int, image_height: int, augmentation_path: str, scale_foreground_by_background_size: bool, scaling_factors: List[int], avoid_collisions: bool, parallelize: bool):
        ...
        self.input_dir1 = Path(input_dir + '/foregrounds/')
    ...
    def _validate_and_process_foregrounds(self):
        ...
        for category in self.foregrounds_dir.glob('*'):
            # check if we have a directory 需要在foregrounds里面再添加文件夹（文件夹名字对应相应的label信息）
        ...
        # print(self.foregrounds_dict)
        # print(self.input_dir1)
    ...
    def _generate_image(self, image_number: int):
        ...
        save_filename = f'{image_number:0{self.zero_padding}}'                                     #文件名字
    ...
    def _compose_images(self, foregrounds, background_image_path):
        ...
        # Resize background to desired width and height 大于指定的尺寸的话就会直接在中间随机的裁剪一块指定的大小的图片出来
        bg_width, bg_height = background_image.size

        if bg_width >= self.image_width and bg_height >= self.image_height:     #背景图片尺寸大于指定的大小就裁剪
        ...
        for fg in foregrounds:
            ...
            if not self.scale_foreground_by_background_size:
                ...
                # Scale the foreground based on the size of the resulting image  先取最短的
    ...
    def generate_images(self):
        if self.parallelize:
            ...
            for i in tqdm(range(1, self.image_number + 1)):
                self._generate_image(i + 50)         #大概在这里加上之前生成图片的id数就可以接上id了hhhhh
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_width', type=int, default=1920, help='Width of the output images')
    parser.add_argument('--image_height', type=int, default=1080, help='Height of the output images')
